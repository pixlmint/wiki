<template>
    <div class="board-list">
        ...
        <draggable
            :class="{ 'items-list': true, 'empty-list': cards.length === 0 }"
            @dragstart="emit('startDragging')"
            @dragend="emit('stopDragging')"
            @change="updateList"
            :list="cards"
            group="board"
            itemKey="name"
        ...
        <div class="mt-1 d-flex justify-content-end">
            <el-input
                v-show="data.addingItem"
                ref="addItemInput"
                v-on:keyup.esc="cancelAddItem"
                v-on:keyup.enter="addItem"
                v-model="data.newItemText"
            ></el-input>
...
const addItem = function () {
    boardStore
        .createListItem(props.list.id, data.newItemText)
