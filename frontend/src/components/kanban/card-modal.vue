        ...
        <el-form>
            <el-form-item label="Labels">
                <div class="d-flex gap-1">
                    <el-select
                        @change="settings.labelsChanged = true"
                    ...
                    <el-button
                        @click="saveSelectedLabels"
                        v-show="settings.labelsChanged"
                    >
                        <pm-icon icon="save"></pm-icon>
                    </el-button>
                    <pw-md-editor
                        editorHeight="500px"
                        v-if="settings.contentLoaded"
                        @input="updateContent"
                        @save="saveCardMdContent"
                        v-model="cardMdContent"
                    ></pw-md-editor>
                </div>
            </el-form-item>
        </el-form>
...
const request = buildRequest("/api/entry/view", {
    p: dialogStore.getDialogData(route).id,
});
...
window.setTimeout(function () {
    ...
    if (typeof viewingCard.value.meta.card !== "undefined") {
        selectedLabels.value = viewingCard.value.meta.card.labels.map(
            (label: CardLabel) => {
                return label.title;
            },
        );
