<template>
    <div class="article-head">
        <div class="d-flex" style="align-items: center; gap: 1rem">
            ...
            <pm-icon
                v-if="!isPublic"
                title="This Entry is Private, only you can see it"
                icon="lock"
            ></pm-icon>
        ...
        <div v-if="canEdit" class="print-invisible">
            <el-dropdown split-button trigger="click" @click="editEntry">
                ...
                <template #dropdown>
                    ...
                    <el-dropdown-item
                        v-if="props.displayViewMarkdownButton"
                        @click="viewMarkdown"
                        title="View"
                    >
                    ...
                    <el-dropdown-item
                        v-if="props.displayMediaButtons"
                        @click="uploadMedia"
                        title="Media"
                    >
                    ...
                    <el-dropdown-item
                        v-if="props.displayDeleteButton"
                        class="danger"
                        @click="deleteEntry"
                        title="Delete"
                    >
...
const viewMarkdown = function () {
    const query = queryFormatter({
        ...
        entry: wikiStore.safeCurrentEntry.id,
    });
    window.open(
        location.origin + "/api/admin/entry/view-markdown?" + query,
        "_blank",
    );
};
...
const deleteEntry = function () {
    const doDelete = confirm(
        `Are you sure you want to delete ${wikiStore.safeCurrentEntry.meta.title}`,
    );
    if (doDelete) {
        useWikiStore()
            .deleteEntry(wikiStore.safeCurrentEntry.id)
            .then(() => {
