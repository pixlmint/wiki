<template>
    <div>
        <div class="drawing-toolbar">
            ...
            <div class="drawing-toolbar-section">
                <template v-for="(tool, index) in tools">
                    <div
                        v-show="selectedTool === index"
                        v-if="tool instanceof DrawTool"
                    >
                        <el-select
                            v-model="tool.selectedColorIndex"
                    ...
                    <div
                        v-show="selectedTool === index"
                        v-if="
                            tool instanceof DrawTool ||
                            tool instanceof EraserTool
                        "
                    >
                        <el-select
                            v-model="tool.selectedWeightIndex"
                            placeholder="Stroke Weight"
                        >
                            <el-option
                                v-for="(
                                    weight, wIndex
                                ) in tool.selectableWeights"
                                :key="wIndex"
                                :label="weight"
                                :value="wIndex"
                            />
        ...
        <div v-if="debug.enabled" class="paint-debug">
            <div>
                Mouse Position: {{ debug.mousePosition.x }},
                {{ debug.mousePosition.y }}
            </div>
            <div>Pointer Type: {{ pointerType }}</div>
            <div>
                Polling Rate:
                <el-slider v-model="editorConfig.pollingRate"></el-slider>
            </div>
...
const save = () => {
    ...
    svgData =
        ...
        svgCanvas.attr("width") +
        '" height="' +
        svgCanvas.attr("height") +
        '">' +
        svgData +
        "</svg>";
    const drawing = {
...
class BaseDrawingMode implements DrawingMode {
    ...
    ) => Selection<SVGPathElement, PaintStroke, BaseType, unknown>;
...
class DrawTool
    extends BaseTool
    implements ColorSelectableTool, WeightSelectableTool
{
    selectableColors: StrokeColor[];
    selectedColorIndex: number;
    selectableWeights: number[];
...
const paintStrokes = (
    paths: PaintStroke[],
    sketch: d3.Selection<d3.BaseType, unknown, HTMLElement, any>,
) => {
    currentPath.points = settings.value.selectedMode.pointsTransformer(
        currentPath.points,
    );
...
const distanceBetween = (
    point1: PaintStrokePoint,
    point2: PaintStrokePoint,
) => {
    return Math.sqrt(
...
const isStrokeErased = (stroke: PaintStroke, eraserStroke: PaintStroke) => {
    for (let i = 0; i < stroke.points.length; i++) {
        ...
        const pointErased = eraserStroke.points.some(
            (eraserPoint) =>
                distanceBetween(currentPoint, eraserPoint) <=
                eraserStroke.baseWeight,
        );
...
const isLineSegmentIntersectingCircle = (
    p0: PaintStrokePoint,
    p1: PaintStrokePoint,
    circleCenter: PaintStrokePoint,
    radius: number,
) => {
    // Line segment vector
    ...
    const lengthLineVec = Math.sqrt(
        lineVec.x * lineVec.x + lineVec.y * lineVec.y,
    );
    const dotProduct =
        (toCircleVec.x * lineVec.x + toCircleVec.y * lineVec.y) / lengthLineVec;
    ...
    const distanceToLine = Math.sqrt(
        (circleCenter.x - closestPointOnLine.x) *
            (circleCenter.x - closestPointOnLine.x) +
            (circleCenter.y - closestPointOnLine.y) *
                (circleCenter.y - closestPointOnLine.y),
    );
...
const modeFreehand = new BaseDrawingMode(
    "Freehand",
    "hand",
...
const modeRectangle = new BaseDrawingMode(
    "Rectangle",
    "hand",
    (points: PaintStrokePoint[]) => {
        if (points.length < 2) {
...
const modeCircle = new BaseDrawingMode(
    "Circle",
    "circle",
    (points: PaintStrokePoint[]) => {
        const bb = new BoundingBox();
...
const modeErase = new EraserTool(
    "Erase",
    "eraser",
    () => {},
    [2, 30],
    undefined,
    (sketch: any, tool: Tool) => {
        paths = eraseStrokes(currentPath);
        reDrawSvg(paths, svgCanvas);
    },
);

const modeDraw = new DrawTool(
    "Draw",
    "pencil",
    drawTempStroke,
...
const modeHighlight = new DrawTool(
    "Highlight",
    "highlighter",
    drawTempStroke,
    [hColorYellow, hColorGreen, hColorPurple],
    [10, 20, 50],
    undefined,
    paintStrokes,
);
...
onBeforeUnmount(() => {
    ...
    const c: HTMLElement | Element | null =
        document.querySelector("svg.d3-canvas");
...
const baseMoveEvent = (event: PointerEvent) => {
    if (isDrawing) {
        ...
        if (currentMove - lastMove >= editorConfig.value.pollingRate) {
            ...
            settings.value.selectedTool.drawFunction(
                svgCanvas,
                settings.value.selectedTool,
            );
...
const pointerDownEvent = (event: PointerEvent) => {
    ...
    if (pointerType === "pen" || pointerType === "mouse") {
        if (settings.value.selectedTool.startModifyFunction !== undefined) {
            settings.value.selectedTool.startModifyFunction(
                svgCanvas,
                settings.value.selectedTool,
            );
        }
        ...
        let baseColor = "#000";
        if (
            "selectableWeights" in settings.value.selectedTool &&
            "selectedWeightIndex" in settings.value.selectedTool
        ) {
            baseWeight =
                settings.value.selectedTool.selectableWeights[
                    settings.value.selectedTool.selectedWeightIndex
                ];
        }
        if (
            "selectableColors" in settings.value.selectedTool &&
            "selectedColorIndex" in settings.value.selectedTool
        ) {
            baseColor =
                settings.value.selectedTool.selectableColors[
                    settings.value.selectedTool.selectedColorIndex
                ].hex;
        }
        currentPath = {
            points: [],
